...
use crate::diagnostic::{CompilerDiagnostic, SourceLocation, SyntaxError};
    ...
    while i < len {
        ...
        if c == b'"' {
            ...
            if i >= len {
                // Unterminated string
                out.errors.push(
                    SyntaxError::unterminated_delimiter(
                        "\"",
                        SourceLocation::new(string_start_line, string_start_col, file),
                    )
                    .into(),
                );
            } else {
    ...
    *col += 1;
    ...
    if *i >= len {
        out.errors.push(
            SyntaxError::unterminated_delimiter(
                "]",
                SourceLocation::new(body_line, body_col, out.file.clone()),
            )
            .into(),
        );
        return;
